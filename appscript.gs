function doGet(e) { 
  ...
  if (e.parameter == 'undefined') {
    ...
    if (sts_val == 'reg') {
      ...
      var nextRow = getLastRowUIDCol + 1;

      // Write UID in column B only. Columns A and C (Name, RegNo) will be filled manually.
      sheet_user_data.getRange("B" + nextRow).setValue(uid_val);
      result += ",R_Successful";
    ...
    if (sts_val == 'atc') {
      ...
      if (FUID == -1) {
        ...
        var user_name = sheet_user_data.getRange("A" + (FUID + 2)).getValue();  // Column A: Name
        var reg_no = sheet_user_data.getRange("C" + (FUID + 2)).getValue();     // Column C: Registration No
        ...
        if (enter_data == "time_in") {
           // Count how many entries are already made today (Column E = Date)
          var today_count = 0;
          for (var j = 1; j < data.length; j++) {
            if (data[j][4] == Curr_Date) { // Column E = Date
              today_count++;
            }
          }

          var serial_no = today_count + 1;

          sheet_attendence.insertRows(2);
          sheet_attendence.getRange("A2").setValue(serial_no);        // Column A: Entry/Exit No
          sheet_attendence.getRange("B2").setValue(user_name);        // Column B: Name
          sheet_attendence.getRange("C2").setValue(uid_val);          // Column C: UID
          sheet_attendence.getRange("D2").setValue(reg_no);           // Column D: Registration No
          sheet_attendence.getRange("E2").setValue(Curr_Date);        // Column E: Date
          sheet_attendence.getRange("F2").setValue(Curr_Time);        // Column F: Time In

          SpreadsheetApp.flush();

          result += ",TI_Successful," + user_name + "," + Curr_Date + "," + Curr_Time + "," + serial_no;
          return ContentService.createTextOutput(result);
                }
        //..................
        ...
        if (enter_data == "time_out") {
          sheet_attendence.getRange("G" + num_row).setValue(Curr_Time);
          result += ",TO_Successful," + user_name + "," + data[num_row - 1][4] + "," + data[num_row - 1][5] + "," + Curr_Time;
